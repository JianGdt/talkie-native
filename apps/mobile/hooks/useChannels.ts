...
export function useChannels(): UseChannelsReturn {
  ...
  const fetchUserChannels = useCallback(async () => {
    if (!user?.id) return;
    try {
      const joined = await channelService.getUserChannels(user.id);
      setUserChannels(new Set(joined.map((c: any) => c.id)));
    } catch (error) {
  ...
  useEffect(() => {
    if (user?.id) reloadData();
  }, [user?.id]);

  // Sync isActive flag whenever userChannels changes
  useEffect(() => {
